...
while read -p "Select Suezx version
1. CM DTTW & S2W
2. CM NO DTTW & S2W
3. MIUI DTTW & S2W
4. MIUI NO DTTW & S2W
...
function alamat {
		...
		mkdir -p $DIR/$TGL
		cd $DIR	
		COUNT=`ls ~/Kernel/Builds/$DIR/$TGL -l | wc -l`
		NUMBER=$(($COUNT +1))
		NUMB="#$NUMBER"
}
...
function skymo {
		echo "Make dtb & zip"
		cd ~/Kernel/AnyKernel2/modules
		rm *.ko
		cd $KERNEL_DIR
		for i in `find -name *.ko`; do cp $i ~/Kernel/AnyKernel2/modules/; done
		$STRIP --strip-unneeded ~/Kernel/AnyKernel2/modules/*.ko
		$DTBTOOL_DIR/dtbToolCM -s 2048 -d "qcom,msm-id = <" -2 -o arch/arm/boot/dt.img -p /usr/bin/ arch/arm/boot/
		cp arch/arm/boot/zImage ~/Kernel/AnyKernel2/
		cp arch/arm/boot/dt.img ~/Kernel/AnyKernel2/
		cd ~/Kernel/AnyKernel2
		DATE=$(date +"%d%m%y")
		rm *.zip
		zip -r9 $SKYMO-$DATE-$NUMB.zip * -x README $SKYMO-$DATE-$NUMB.zip
		cp *.zip ~/Kernel/Builds/$DIR/$TGL
}
